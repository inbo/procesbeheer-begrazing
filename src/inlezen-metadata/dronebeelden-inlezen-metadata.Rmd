...
```{r}
calc_area_classes <- function(chm, cuts, grts_raster, bufferdist) {
  ...
  # reclassify canopy heights
  chm_classes <- classify(chm, cuts, include.lowest = TRUE)
  
  # convert to RasterLayer
  chm_classes <- raster::raster(chm_classes)
  
  # extract area covered by classes within buffer
  chm_class_areas <- exactextractr::exact_extract(
    chm_classes, 
    grts_buffer,
    function(values, coverage_fraction)
      sum(values * coverage_fraction, na.rm = TRUE))
  
  return(chm_class_areas)
}
```


```{r}
height_classification <- c(0, 0.07, 0.15, 0.25, 0.45, 0.70, 1, 1.5, 2, 2.75, 4, 8)
ndvi_classification <- c(0, 0.1, 0.2, 0.3, 0.4)
bufferdistance <- 5
